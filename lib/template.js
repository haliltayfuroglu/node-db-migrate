...
Template.prototype.bulkSqlLoaderTemplate = function () {
  return [
    "'use strict';",
    '',
    'var dbm;',
    ...
    "var fs = require('fs');",
    "var path = require('path');",
    "var { dbMigrationParser }  = require('./commands/helper/migration-parser.js');",
    'var Promise;',
    '',
    '/**',
    '  * We receive the dbmigrate dependency from dbmigrate initially.',
    ...
    "    fs.readFile(filePath, {encoding: 'utf-8'}, function(err,data){",
    '      if (err) return reject(err);',
    '      resolve(dbMigrationParser(data))',
    '    });',
    '  })',
    '  .then(function(data) {',
    '      if (data && data.length > 0) {',
    '        return Promise.all(data.map(sql => db.runSql(sql)))',
    '      }',
    '  });',
    '};',
    '',
...
Template.prototype.getTemplate = function () {
  ...
  switch (this.templateType) {
    ...
    case Template.TemplateType.BULK_SQL_LOADER:
      return this.bulkSqlLoaderTemplate();
    default:
      if (this.plugins) {
