...
module.exports = function (internals, isModule) {
  ...
  if (!isModule) {
    internals.argv = yargs
      ...
      .boolean('bulk-sql-file')
      .describe(
        'bulk-sql-file',
        'Automatically create two sql files for up and down statements in ' +
          '/sqls and generate the javascript code that loads them.' +
          'It can be contains multiple sqls seperated by --'
      )
      .boolean('bulk-sql-file')
      ...
      .describe(
        'state-table',
        'Set the name of the migration state table, which stores the migration' +
          ' state to coordinate concurrency and advanced features.'
      )
      .alias('state', 'state-table')
      .alias('s', 'state')
      .string('s')
      .string('seeds-table')
      .describe(
  ...
  internals.migrationState = internals.argv.state;
