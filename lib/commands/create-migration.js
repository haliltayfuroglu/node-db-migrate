...
async function executeCreateMigration(internals, config) {
  ...
  } else if (shouldCreateBulkSqlFiles(internals, config)) {
    console.log("***********************************HALO*************");
    templateType = Migration.TemplateType.BULK_SQL_LOADER;
  ...
  if (!pluginTemplate && !customWrite) {
    ...
    if (shouldCreateSqlFiles(internals, config) || shouldCreateBulkSqlFiles(internals,config)) {
...
function shouldCreateBulkSqlFiles(internals, config) {
  return internals.argv['bulk-sql-file'] || config['bulk-sql-file'];
}
